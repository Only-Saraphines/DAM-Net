...
# 设置文件路径和参数
data_dir = 'D:\ADNI_brain_data_sets\\CNMCI_classify_data\All\\train'  # 数据集路径
data_dir_Ctx = 'D:\ADNI_brain_data_sets\\CNMCI_classify_data\Ctx\\train'
data_dir_Hip = 'D:\ADNI_brain_data_sets\\CNMCI_classify_data\Hip\\train'
batch_size = 16  # 批量大小
num_epochs = 1 # 训练的轮数
...
# 测试模型
test_dataset = BrainDataset('D:\ADNI_brain_data_sets\\CNMCI_classify_data\All\\val')
test_dataloader = DataLoader(test_dataset, batch_size=batch_size, shuffle=True)

# 创建数据集实例和数据加载器
Alldataset = BrainDataset(data_dir)

Ctxdataset = BrainDataset(data_dir_Ctx)
...
Ctxdataloader = DataLoader(Ctxdataset, batch_size=batch_size, shuffle=True)

Hipdataloader = DataLoader(Hipdataset, batch_size=batch_size, shuffle=True)
# 创建模型实例和损失函数
model = MultiTaskModel(2)
...
for epoch in range(num_epochs):
    ...
    for (images1, labels1), (images2, labels2), (images3, labels3) in zip(Alldataloader, Ctxdataloader, Hipdataloader):
        ...
        _, _, temporal_aux_outputs = model(images3)
        # print('temporal_aux_outputs:', temporal_aux_outputs)
        # 将标签转换为独热编码
        class_labels = ['CN', 'MCI']
        ...
        temporal_aux_loss = criterion(temporal_aux_outputs, onehot_labels3)
        # print('temporal_aux_loss:', temporal_aux_loss)
        # 总体损失为主分类任务损失加上辅助任务损失
        total_loss = main_loss + 1.0 * hippocampus_aux_loss + 0.7 * temporal_aux_loss  # 超参数加权求和
